...
const { resolveSpill } = require('./intent-spill');
...
function resolveFilePath(name, raw, baseDir) {
  ...
  if (typeof raw === 'string' && /^@spill:[0-9a-f]{16}$/.test(raw.trim())) {
    const sp = resolveSpill(REGISTRY_DIR, name, raw.trim().slice(7));
...
// create() falls back to Date.now(), re-minting it and reordering "created" sort.
    ...
    const created = await manager.create(name, entry.type, manager.resumeCwdOf(entry), entry.extraArgs || [], resumeId, wsId, entry.systemPrompt || null, false, entry.proxy ?? null, entry.agents || [], entry.denyBuiltins || [], entry.disabledTools || [], entry.disabledSkills || [], entry.injectSkills || [], entry.systemPromptFile || null, entry.appendPromptFiles || [], Array.isArray(entry.execCommands) ? entry.execCommands : [], Array.isArray(entry.intents) ? entry.intents : null, (entry.env && typeof entry.env === 'object') ? entry.env : null, false, entry.noWire === true, Array.isArray(entry.plugins) ? entry.plugins : null, Array.isArray(entry.shellDeny) ? entry.shellDeny : null);
