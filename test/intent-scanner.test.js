...
test('parseIntent: task add — bracket-arg optional (backlog vs mint+assign)', () => {
  assert.deepStrictEqual(parseIntent('[agent:task add] build the widget'),
    { type: 'task', sub: 'add', who: null, id: null, park: false, start: false, reviewer: null, body: 'build the widget' });
  assert.deepStrictEqual(parseIntent('[agent:task add hand] build the widget'),
    { type: 'task', sub: 'add', who: 'hand', id: null, park: false, start: false, reviewer: null, body: 'build the widget' });
});
...
test('parseIntent: task add park — the modifier never lands in `who` (t174)', () => {
  ...
  assert.deepStrictEqual(parseIntent('[agent:task add park hand] spec'),
    { type: 'task', sub: 'add', who: 'hand', id: null, park: true, start: false, reviewer: null, body: 'spec' });
  ...
  assert.deepStrictEqual(parseIntent('[agent:task add park] spec'),
    { type: 'task', sub: 'add', who: null, id: null, park: true, start: false, reviewer: null, body: 'spec' });
  ...
  assert.deepStrictEqual(parseIntent('[agent:task add parked] spec'),
    { type: 'task', sub: 'add', who: 'parked', id: null, park: false, start: false, reviewer: null, body: 'spec' });
});

test('parseIntent: task add start — position-free, and never the assignee (t897)', () => {
  assert.deepStrictEqual(parseIntent('[agent:task add start hand] spec'),
    { type: 'task', sub: 'add', who: 'hand', id: null, park: false, start: true, reviewer: null, body: 'spec' });
  assert.deepStrictEqual(parseIntent('[agent:task add hand start] spec'),
    { type: 'task', sub: 'add', who: 'hand', id: null, park: false, start: true, reviewer: null, body: 'spec' });
  assert.deepStrictEqual(parseIntent('[agent:task add start hand] spec'),
    parseIntent('[agent:task add hand start] spec'));
  assert.deepStrictEqual(parseIntent('[agent:task add start] spec'),
    { type: 'task', sub: 'add', who: null, id: null, park: false, start: true, reviewer: null, body: 'spec' });
  assert.deepStrictEqual(parseIntent('[agent:task add started] spec'),
    { type: 'task', sub: 'add', who: 'started', id: null, park: false, start: false, reviewer: null, body: 'spec' });
  assert.deepStrictEqual(parseIntent('[agent:task add hand park start] spec'),
    { type: 'task', sub: 'add', who: 'hand', id: null, park: true, start: true, reviewer: null, body: 'spec' });
  assert.deepStrictEqual(parseIntent('[agent:task add start hand reviewer:rv-shell] spec'),
    { type: 'task', sub: 'add', who: 'hand', id: null, park: false, start: true, reviewer: 'rv-shell', body: 'spec' });
  assert.deepStrictEqual(parseIntent('[agent:task start t7]'),
    { type: 'task', sub: 'start', id: 't7', who: null, reviewer: null, body: '' });
});

test('shadowIntentKey: the start modifier discriminates one add from another (t897)', () => {
  const held = parseIntent('[agent:task add hand] spec');
  const dispatched = parseIntent('[agent:task add hand start] spec');
  assert.notStrictEqual(shadowIntentKey('lead', held), shadowIntentKey('lead', dispatched),
    'a held add and a dispatching add in one turn are two emissions, not a double-paste');
});
...
test('parseIntent: task add reviewer: — a KEYED modifier, never the assignee (t673)', () => {
  ...
  assert.deepStrictEqual(parseIntent('[agent:task add reviewer:rv-shell hand] spec'),
    { type: 'task', sub: 'add', who: 'hand', id: null, park: false, start: false, reviewer: 'rv-shell', body: 'spec' });
  ...
  assert.deepStrictEqual(parseIntent('[agent:task add hand park reviewer:rv-shell] spec'),
    { type: 'task', sub: 'add', who: 'hand', id: null, park: true, start: false, reviewer: 'rv-shell', body: 'spec' });
  // No assignee at all is still a legal selection.
  assert.deepStrictEqual(parseIntent('[agent:task add reviewer:rv-shell] spec'),
    { type: 'task', sub: 'add', who: null, id: null, park: false, start: false, reviewer: 'rv-shell', body: 'spec' });
  ...
  assert.deepStrictEqual(parseIntent('[agent:task add reviewer] spec'),
    { type: 'task', sub: 'add', who: 'reviewer', id: null, park: false, start: false, reviewer: null, body: 'spec' });
});
...
test('parseIntent: team role-add — brief BODY + optional prompt/template/dispatch/cwd key:val tokens', () => {
  assert.deepStrictEqual(parseIntent('[agent:team role-add worker] does the widgets'),
    { type: 'team', sub: 'role-add', name: 'worker', prompt: null, template: null, dispatch: null, cwd: null, model: null, account: null, body: 'does the widgets' });
  assert.deepStrictEqual(parseIntent('[agent:team role-add worker prompt:my-p template:tpl] the brief'),
    { type: 'team', sub: 'role-add', name: 'worker', prompt: 'my-p', template: 'tpl', dispatch: null, cwd: null, model: null, account: null, body: 'the brief' });
  assert.deepStrictEqual(parseIntent('[agent:team role-add worker dispatch:worktree cwd:sub] the brief'),
    { type: 'team', sub: 'role-add', name: 'worker', prompt: null, template: null, dispatch: 'worktree', cwd: 'sub', model: null, account: null, body: 'the brief' });
  assert.deepStrictEqual(parseIntent('[agent:team role-add worker model:opus] the brief'),
    { type: 'team', sub: 'role-add', name: 'worker', prompt: null, template: null, dispatch: null, cwd: null, model: 'opus', account: null, body: 'the brief' });
});
...
test('parseIntent: team role-set — same shape as role-add', () => {
  assert.deepStrictEqual(parseIntent('[agent:team role-set worker prompt:new-p] new brief'),
    { type: 'team', sub: 'role-set', name: 'worker', prompt: 'new-p', template: null, dispatch: null, cwd: null, model: null, account: null, body: 'new brief' });
  assert.deepStrictEqual(parseIntent('[agent:team role-set worker dispatch:spawn] new brief'),
    { type: 'team', sub: 'role-set', name: 'worker', prompt: null, template: null, dispatch: 'spawn', cwd: null, model: null, account: null, body: 'new brief' });
  assert.deepStrictEqual(parseIntent('[agent:team role-set worker model:claude-sonnet-5] new brief'),
