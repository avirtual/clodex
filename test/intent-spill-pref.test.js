...
test('the checkbox exists in the prefs markup with the wording the ticket fixed', () => {
  ...
  assert.match(html, /the first two long bodies of a session stay in its transcript in full; from the third on the transcript keeps the intent head, a bracketed runtime note and <code>\[agent:end\]<\/code>, the model sees only the ordinary confirmation/,
    'the first two bodies ride in full and later ones render as the intent around a runtime note, and the hint must say exactly that');
...
test('the web bundle carries the same three halves as the renderer source', () => {
  ...
  assert.match(bundle, /from the third on the transcript keeps the intent head, a bracketed runtime note and <code>\[agent:end\]<\/code>, the model sees only the ordinary confirmation, and the terminal shows the filed path \(clickable\)/,
