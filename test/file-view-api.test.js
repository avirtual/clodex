...
test('peekFile: binary detection and every error code', () => {
  ...
  assert.deepStrictEqual([link.ok, link.code], [false, 'not-a-file'], 'a symlink is refused by lstat, like resolveSpill');
...
test('confinement (remote query only): cwd, spill, messages and task dirs of the RUNNING registry are readable; anything else is outside', () => {
  ...
  assert.ok(home.ok === false && (home.code === 'outside' || home.code === 'not-found'), 'a ~/.clodex path is never a root of a non-default registry');
