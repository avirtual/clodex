...
// happened to contain the one key under test".
const SIBLING = 'CLODEX_SPAWNER_HINT';

function mkManager(scopes) {
...
async function spawnEnv(name, { global = {}, workspaces = {} } = {}, sessionEnv = null) {
  const had = Object.prototype.hasOwnProperty.call(process.env, KEY);
  const prev = process.env[KEY];
  delete process.env[KEY];
  const { m, spawns, stop } = mkManager({ global, workspaces });
  try {
    await m.create(name, 'bash', os.tmpdir(), [], null, 'ws', null, false, null,
      [], [], [], [], [], null, [], [], null, sessionEnv);
    // create() has several throw/early paths before pty.spawn (cwd stat, hook
    // setup). Without this every assertion below would be reading `undefined`
    // property accesses on a missing record, and `undefined === undefined`
    // passes for the override cases.
