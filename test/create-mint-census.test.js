...
const EXPECTED = [
  ...
  { file: 'ipc-handlers.js', mint: null, noWire: null, label: 'team:createBare sandboxed — mgr.create(`team-${name}`, label), the BOX REGISTRY, exactly as sandbox:createBox below. Same object, same reasoning: no mint axis, no PTY. The team\'s own seats are minted INSIDE the box by its seeder, never here.' },
  { file: 'ipc-handlers.js', mint: true, noWire: 'false', label: 'peer:deployFix — brand-new session under a freshly deconflicted name. Never wire-off: it is a support session Clodex authors itself, with no operator checkbox behind it. The `false` is written EXPLICITLY here, unlike the absent-argument rows: this site must reach create()\'s 24th positional (`fixFor`, the host the seat ssh-es into), so every positional before it has to be spelled out. The value is still the same one the default supplied.' },
  ...
  { file: 'remote-wiring.js', mint: false, noWire: 'rec.noWire === true', label: 'importCreate — the far half of Move-to-peer (M-A2). A RESTORE of a record shipped from another box, not a birth: the seat keeps its sessionId, its transcript is installed under the far encoding of the far cwd before this runs, and its promptcache travelled with it — which is the entire reason mint must be false here. A mint would regenerate the frozen prompt the resume delta is computed against, paying the full rewrite the cache exists to stop. Refuses live-or-persisted names like the createSession row below, but for collision, not because the seat is new. Sorts FIRST of the two rows in this file: it is the earlier call site.' },
  ...
  { file: 'session-manager.js', mint: false, noWire: 'entry.noWire === true', label: 'rename(name, newName) — Rename…, the SAME seat respawned under a new name. NOT a mint on the same reasoning as move: the record is rewritten in place rather than dropped, the conversation is carried by --resume, and the destination name is refused if anything already holds it. Sorts FIRST of the three rows in this file — it is the earliest call site.' },
  { file: 'session-manager.js', mint: false, noWire: 'entry.noWire === true', label: 'move(name, newCwd) — Move Session…, the SAME seat respawned in a new folder. NOT a mint: the record is never dropped (move deliberately avoids kill()), the conversation is carried by --resume, and the only field that changes is cwd. Sorts before the reload row: both are in this file and this one is the earlier call site.' },
  { file: 'session-manager.js', mint: false, noWire: 'entry.noWire === true', label: 'moveToPeer(name, peerId) — the FAILURE arm of Move to a peer, which brings the same seat back here exactly as a failed local move does. NOT a mint for move()\'s reason and one more: the far side refused, so nothing about this record changed and the seat is resuming the conversation it was quiesced out of. Sorts after the move row and before the reload row — all three are in this file, in call-site order.' },
  { file: 'session-manager.js', mint: false, noWire: 'entry.noWire === true', label: '[agent:context reload] — cold respawn of the SAME seat' },
  { file: 'session-manager.js', mint: false, noWire: 'entry.noWire === true', label: '[agent:scratch end] — the cut\'s respawn of the SAME seat on its truncated transcript. NOT a mint, and the most emphatically so of any row here: it passes `entry.sessionId` and resumes the very conversation whose bytes it just shortened, under a name and a record that were never dropped. A mint would regenerate the frozen prompt and pay the full rewrite on a seat whose whole purpose was to SHRINK what it re-reads. Sorts last of the five rows in this file — it is the latest call site.' },
  ...
  // Both rows below spell noWire `'false'` — the literal written out, not the
  // default taken — because each must reach a LATER positional: the reviewer's
  // 23rd (`shellDeny`) and the ticket seat's 22nd (`plugins`, the template's
  // checked bundle list). The literal is what keeps that reach from turning into
  // an authority change: a template is agent-writable, so neither site may read
