...
const EXPECTED = [
  ...
  { file: 'session-manager.js', mint: true, noWire: false, label: 'branch-per-ticket seat — [agent:task add] to a role with `worktree: true` mints a one-shot seat in its own git worktree. Always new (the name carries the ticket number and a taken name aborts the dispatch). Never wire-off: it is a working seat Clodex spawns on the lead\'s behalf, with no operator checkbox behind it, and the same reasoning as the reviewer row.' },
