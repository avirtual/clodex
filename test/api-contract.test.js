...
const PINNED_NAMES = [
  ...
  'teamPreflight',
  // t701: Gather — the one action that fills teams/<name>/ from the library. A
  // WRITE, unlike teamPreflight above it, and `{ dry: true }` is what the roles
  // popover shows before the operator commits.
  'teamGather',
  ...
  'saveTemplateByName', 'removeTemplate', 'saveTeamTemplate', 'removeTeamTemplate',
  'exportTemplate', 'listPrompts',
  'savePrompt', 'removePrompt', 'saveTeamPrompt', 'removeTeamPrompt', 'injectPrompt', 'listAgents',
  'getAgent', 'saveAgent', 'removeAgent', 'listSkillLib',
  'getSkillLib', 'saveSkillLib', 'removeSkillLib', 'listExecCommands',
  'getExecCommand', 'saveExecCommand', 'removeExecCommand', 'listNotifications', 'pageNotifications',
  'markNotificationRead', 'markAllNotificationsRead', 'removeNotification', 'notificationUnreadCount',
  // t806: any inbox mutation from any surface, phone included — the desktop
  // badge's only signal for a read that happened off-box.
  ...
  'onRequestOpenPreferences', 'onRequestOpenHelp', 'onRequestOpenPeersDialog', 'onRequestOpenPeerSession', 'onRequestOpenAgentsDrawer',
  ...
  'onSetTheme', 'setSettings', 'getSetupState', 'completeSetup', 'onZoomNudge', 'setDefaultToolDeny',
  'setDefaultSkillDeny', 'setDefaultBuiltinDeny',
  ...
  // t811: registered Claude subscriptions — the registry behind every seat's
  // CLAUDE_CONFIG_DIR plus the bulk model->account move. Bound on both surfaces;
  // the handlers are gated on enableAccounts, which the web host declines.
  'accountsList', 'accountsAdd', 'accountsRemove', 'accountsResync',
  'accountsMoveByModel',
  ...
  // t679: the app editing a plugin's own content files, refused for every root
  // but the user's. Appended deliberately as the surface grew.
  ...
  // t785: the read-only per-role activity channel behind the roles popover.
  'teamActivity',
  'helpIndex', 'helpPage',
];
...
test('contract covers exactly the pinned 305-method surface', () => {
  assert.equal(PINNED_NAMES.length, 305, 'pinned list is the full 305-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 305, 'window.api has exactly 305 methods');
