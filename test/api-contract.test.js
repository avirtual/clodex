...
test('contract covers exactly the pinned 295-method surface', () => {
  assert.equal(PINNED_NAMES.length, 295, 'pinned list is the full 295-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 295, 'window.api has exactly 295 methods');
