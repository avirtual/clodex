...
test('contract covers exactly the pinned 261-method surface', () => {
  assert.equal(PINNED_NAMES.length, 261, 'pinned list is the full 261-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 261, 'window.api has exactly 261 methods');
