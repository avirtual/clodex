...
test('contract covers exactly the pinned 165-method surface', () => {
  assert.equal(PINNED_NAMES.length, 165, 'pinned list is the full 165-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 165, 'window.api has exactly 165 methods');
