...
test('contract covers exactly the pinned 291-method surface', () => {
  assert.equal(PINNED_NAMES.length, 291, 'pinned list is the full 291-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 291, 'window.api has exactly 291 methods');
