...
test('contract covers exactly the pinned 242-method surface', () => {
  assert.equal(PINNED_NAMES.length, 242, 'pinned list is the full 242-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 242, 'window.api has exactly 242 methods');
