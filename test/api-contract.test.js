...
test('contract covers exactly the pinned 243-method surface', () => {
  assert.equal(PINNED_NAMES.length, 243, 'pinned list is the full 243-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 243, 'window.api has exactly 243 methods');
