...
test('contract covers exactly the pinned 262-method surface', () => {
  assert.equal(PINNED_NAMES.length, 262, 'pinned list is the full 262-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 262, 'window.api has exactly 262 methods');
