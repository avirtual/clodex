...
test('the voice island registers no DOM listener that accepts a parameter', () => {
  ...
  // ENTER: the by-name registrations must have survived too. The rule below is a
  // universal over THIS subset, and today the island happens to contain exactly
  // one — so if the named-callback resolution broke, the rule would pass over an
  // empty set while the inline count above still looked healthy.
  ...
  // The rule itself: a callback registered by NAME must take no parameters.
