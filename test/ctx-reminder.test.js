...
test('the 150k nudge boundary holds from both sides', () => {
  ...
  assert.ok(ctxReminderFor(199_999).includes('getting heavy'), 'still only the nudge just under escalate');
  assert.ok(!ctxReminderFor(199_999).includes('very heavy'), 'and not yet escalated');
  assert.ok(ctxReminderFor(200_000).includes('very heavy'), 'ENTER: 200k is the escalate boundary');
});
...
test('the token count is rendered in ~Nk form', () => {
  assert.ok(ctxReminderFor(160_000).includes('~160k'));
