...
- Internal: transcript parsing is one reader per platform (`transcript-readers.js`); Muse Code transcripts parse.
...
- A sandbox box is now owned by the Clodex instance that composed it: `up`, `rebuild` and `down` refuse a box whose docker project was created by another instance's compose file (`code: foreign`), and the Sandboxes panel shows such a box read-only with its owner's path — a second Clodex on the same Mac can no longer stop the first one's live box through a same-named row.
- Each sandbox box gets its own web-console token, minted into its `auth.env` beside the peer-wire token and recorded as `webToken` in the team's `sandbox.json`; the Teams ▸ "<team> — sandboxed" menu row opens the console authenticated. The box no longer inherits `CLODEX_WEB_TOKEN` from the host process's environment.
...
- A spilled body's stand-in in the terminal now reads `<head> <first line> — 5.2 KB filed at <path>` instead of a bare `@spill:<id>` token; the path is clickable and the file name is unchanged. The model never saw either.
- Long intent bodies written in reply to an injected turn (a dm, a reminder, a ticket or exec notice) are now spilled like any other: the request editor keeps a one-line placeholder where such a turn's only content was a stub, instead of leaving the whole body in the transcript. Before this, a lead's ticket specs — almost always written in reply to a notice — never spilled at all.
...
- Spill pointers, receipts and runtime notes that earlier versions wrote into an agent's transcript are now removed from every API request before it leaves the box, so the model never sees its own stand-in again; the transcript on disk and the terminal are unchanged. The transcript now keeps a short `@spill:<id>` pointer where a long body was filed — clickable in the terminal — and a pointer typed by an agent in place of a body is refused as not executed.
...
- The IPC Traffic log now shows a row for every spill the wire tee files (`clodex → <agent>: <head> (N B) filed at <path>`), and the path is clickable — it opens the spilled body in the file viewer. Until now a spill was visible only in the agent's own prompt ack.
...
- **`clodexctl logs` gains `--since` and `--timestamps`, and `logs node` tails the node's own log.** `--since 30m` (or an ISO time) keeps only newer turns, `--timestamps` prefixes each turn with its wall-clock time, and `logs node` prints the engine's own log so a headless node can be checked without a shell on it.
- **A modern chrome around the terminal.** The sidebar, dialogs, popovers, toasts and status bar move to one token set with dark and light palettes, quieter borders and a consistent type scale; every dialog gains a close button that behaves exactly like Escape. The terminal pane itself is unchanged.
...
- **Nodes are a resource too.** The nodes `clodexctl` can talk to (what used to be contexts) are `get nodes` (`--current` for the active one), `describe node <name>` (`--test` opens the transport and says hello), `create node <name> --url|--ssh|--ssm|…` (`--import` seeds them from the local GUI), `delete node <name>` and `use node <name>`. The `ctx` family is gone; each old spelling prints the new one and exits 1. Node output never carries a token in any format.
...
- `clodexctl exec <agent> "…"` and `logs -f` work on sessions with a long history: both now follow the transcript by sequence number instead of by count, so an agent that has been running for a while no longer answers with silence, and `logs -f` keeps following across a session restart.
...
- Fixed: the read-only lock on the mount path field in Preferences ▸ Phone
  ...
  Until now the field still went read-only saying the box served the variable's
  value, and quietly dropped any edit you saved. In exactly those cases the
  field now stays editable and your save takes effect. The "Applies as soon as
  you save" line is also hidden while the field really is locked, where it
  contradicted the note right below it. Upgrading changes nothing about what any
  box is already serving.
...
- `GET /api/transcript/:name`'s `cursor` is now the start of the last turn (the earliest message that can still change) rather than the last message returned, and the response without `since` carries it too: a reply that re-merges mid-turn shortens the list, and a client polling from the old tail got an empty page and, on the phone page, a blank view. The phone page no longer blanks on an empty page.
- The transcript API and the phone page no longer show a subagent's `<task-notification>` block (task ids, output paths, the whole result body) as a conversation message; it is dropped like the harness's `<system-reminder>` blocks already are, so it no longer splits the reply it interrupted into two bubbles either.
...
- The "running host booted from OLDER code" note rides only the Clodex team's own ticket replies: a lead on another project never sees a warning it cannot act on.
...
- A team's own prompts now show up everywhere its templates already did: the Prompts drawer lists them under "Team <name>" (with a "Shadowed by team" note on the library copy), Edit saves into the team's directory, and the role prompt picker in the team popover offers them under a Team group instead of flagging them missing.
- The team popover's template field is now a picker: a role's own template (Team group), the library's, and plugin templates, with an Open button that jumps into the template editor. Changing a team's hand model is: open the team popover, Open the hand's template, set the model, save.
...
- **Teams:** a ticket's merge notice on a repo with no CHANGELOG.md now says so, instead of telling the lead on every merge that an entry is owed.
- **Teams:** the loop's reviewer now boots inside the ticket's worktree, with the ticket's task dir (spec and diff) added as a readable directory, so a reviewer running without permission bypass reads its subject without a directory-access prompt. Before, it booted at the team root and the CLI blocked every read of the worktree until the operator granted it by hand. A manual `[agent:team-review]` is unchanged.
- **Teams:** `[agent:team create <name> root:<path>]` takes a body: the kickstart brief. With one, the team is born with its hand per-ticket (a branch, worktree and seat per ticket) and the brief saved as `prompts/append/team-project.md`, which the stock lead and hand templates read at boot — the two manual follow-up steps the docs used to list. Without a body nothing changes. A brief that cannot be saved leaves no team behind.
...
- **Teams:** the stock hand template is lean: `--model claude-opus-5` and every skill off. `"disabledSkills": ["*"]` in any template turns off every skill Clodex has seen on this box at spawn time (injected skills excepted), so a template can say "no skills" without naming them.
...
- The wirescope `/_session` page renders each tool call's inputs as fields (a Bash description, a file path, a search pattern) instead of one JSON blob — wirescope v0.6.63 vendored.
...
- The Plugins dialog's inline buttons — Cancel / Resolve / Install under Install from GitHub…, each plugin's Settings / Update / Unregister and the settings panel's Save — use the dialog's own button style instead of the browser default, with Cancel on the left and the accent-coloured Install on the right.
...
- **Live Bash output now shows up for concurrent calls that run the same
  ...
  now stream, each row carrying its own call's output. Calls that genuinely
  cannot be told apart still refuse rather than guess.
