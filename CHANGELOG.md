...
- A ticket whose automatic checks fail now stays recoverable instead of going
  ...
  time while its checks are still running is now told exactly that, rather than a
  bare "is done, not open" that left rejecting the ticket as the only obvious move.
