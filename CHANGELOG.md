...
- `clodexctl exec <agent> "…"` prints the reply again on any agent whose transcript is long: the client now follows the transcript by sequence number instead of by count, so a session with a long history no longer answers with silence.
