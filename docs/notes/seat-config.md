...
- The seat dir is `run/<name>/xdg` (`KINDS.seatConfig`): rebuilt with `rm -rf` + mkdir on every `create()`, so nothing under it can outlive a restart by construction — that is the `run/<name>/` invariant, held without a sweep.
