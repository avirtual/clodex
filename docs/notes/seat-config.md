...
## newestMuseTranscript
- The deadline fallback for a seat whose registry record never lands (the `/private/tmp` refusal above): the newest `session.jsonl` under `<data>/muse/sessions/**` whose mtime is at or after the seat's `spawnedAt`, minus every path another live Muse seat's `run/<name>/transcript.jsonl` already points at. Measured on 1.3.0 the file lands ~0.25 s after spawn. The age compare gives 1 s of slack below `spawnedAt` because filesystems with 1-second mtime granularity truncate a write made 0.25 s after the spawn to a second before it. `untilMs`, the earliest `spawnedAt` of any OTHER live Muse seat spawned after this one, is an exclusive upper bound: a file at or after it may be that seat's, so two seats within one deadline window link nothing rather than each other's log.
