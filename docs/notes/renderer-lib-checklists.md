...
only when the set handed in already named it AND the seat still holds the
plugin, so un-holding a plugin in Edit Session drops its prompts at save
instead of persisting a ref the next start would refuse.
