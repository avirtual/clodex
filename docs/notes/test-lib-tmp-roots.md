...
The `ROOTS` membership check is what keeps this from being a second unswept
route into `$TMPDIR`: without it `mkTmpDirIn(os.tmpdir(), p)` mints an
untracked root and passes the source-shape pin green, which is the exact
invariant the pin exists to hold.
