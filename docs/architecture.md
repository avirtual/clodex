...
- **team-preflight.js** — "does this team's manifest name anything that resolves
  ...
  probes — no fs, its one require being `plugin-prompt-refs` — which is what makes
  the whole findings table assertable with no library installed. What it resolves
  per role: the role's `prompt`, its template, that template's `systemPromptFile`
  and `appendPromptFiles` stems, and every exec def the template grants (plus the
  `${TEAM_ROOT}` paths inside those defs). A template stem that is a
  plugin-namespaced ref by `splitPluginPromptRef` — the runner's own rule — is the
  plugin's to resolve and is checked at neither site. Absence of a finding for a role is
...
- **team-template-derive.js** — `resolveModelId(v)` / `deriveModelTemplate(base,
  ...
  `resolveModelId` is an ALLOWLIST — four aliases, else an
  `[a-zA-Z0-9][a-zA-Z0-9.-]{0,63}` id with at most one trailing lowercase `[…]`
  suffix — because the value reaches a spawn's argv, so the brackets are legal
  ONLY in that trailing position and a stem containing one is still refused. The
  suffix is lowercase-only because `argv-merge.js` matches `/\[1m\]$/` case-
  sensitively, so an accepted `[1M]` would keep a 200k denominator. Three of the
  aliases now resolve to `[1m]` ids, and the alias is the only way to reach one:
  a bracketed id typed into the kv is unreachable, since the intent's family
  regex stops at the first `]` and delivers a truncated `claude-opus-5[1m` the
  allowlist then refuses — which is why the aliases exist.
  Derivation drops every prior `--model <x>` pair and `--model=<x>`
...
- **meta/session dialog leaves**: **args-model.js** (the Model field as a VIEW
  ...
  **plugin-source-dialog.js** (Manage Plugins ▸ Install from GitHub… and a
  fetched row's Update…: the preview lines for both, the source line under the
  row's name, the trust warning naming repo/ref/commit, and whether the button
