...
- **Spill-stub cut** (wire/spill-cut.js): shadow records `wire-spill-cut` (agent, reqId, lines/blocks/messages/placeholders counts; `placeholders` is the subset of `messages` kept as a head-line placeholder behind a `role:"system"` row rather than dropped) per edited request, plus `wire-spill-cut-error` (agent, reqId, error) when the editor throws and the request is forwarded uncut; one `intent` line each in clodex.log, no PTY injection.
