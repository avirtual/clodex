...
- **Spill filing** (wire/spill.js → session-manager `wire.on('spill')`): shadow records `wire-spill` per filed body, a prose spill gets the agent a `[clodex] … filed at …` notice on its next prompt while an intent body gets no notice and its transcript stand-in is the runtime note (the first two bodies of a session are exempt and ride in full), and the filing also lands as a `spill` ipc-message row in the IPC traffic tab (`clodex → <agent>`, with the file path clickable) so the operator sees every spill without reading the shadow log; the transcript keeps `<head> [<title> — ]<size> filed at <path>` in the body's place, the same path, and clodex.log's `intent` line names it too. The tee holds `task add/respec/reject/done`, `dm` and `shout`; a `context compact|clear|reload` handoff is never spilled, because a body the triggering action discards is never carried forward, so there is nothing to save.
- **Prompt regeneration** (session-manager `_handleContextIntent` clear arm, `_compactRegen`): shadow records `prompt-regen-at-clear` {agent, bytes} when a `[agent:context clear]` on a seat with a pending instruction change cold-respawns it on a regenerated prompt, and `prompt-regen-at-compact` {agent, bytes} when a compact summary landing on such a seat respawns it with `--resume` and a fresh bake; bytes is the regenerated realIpc size. `prompt-refresh-error` {agent, error} when the predicate could not assemble the prompt.
- **Spill-stub cut** (wire/spill-cut.js): shadow records `wire-spill-cut` (agent, reqId, lines/blocks/messages/placeholders counts; `placeholders` is the subset of `messages` — pointer-only, no head to render — kept as a third-person placeholder behind a `role:"system"` row rather than dropped) per edited request, plus `wire-spill-cut-error` (agent, reqId, error) when the editor throws and the request is forwarded uncut; one `intent` line each in clodex.log, no PTY injection.
