...
`BOOT_NUDGE_QUIET_MS` (1s) — a `--resume` replaying a long transcript brings its
readline loop up after `BOOT_DRAIN_SETTLE_MS`, so the drain's own Enter is read
as pasted content and the message sits in the composer unsubmitted. The nudge is
cleared by the turn edge and by kill, gives up at `INJECT_BOOT_MAXWAIT`, and logs
`boot-drain nudge for <seat>` when it fires.
