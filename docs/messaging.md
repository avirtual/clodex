...
| UserPromptSubmit | `run/<name>/poll-guard.sh` | clears `run/<name>/poll-state` — a new turn resets the repeat counter, so an operator's own reply can never be what trips the deny below |
| PreToolUse (`matcher: ''`, all tools) | `run/<name>/poll-guard.sh` | counts CONSECUTIVE identical Bash commands in `poll-state` and returns `permissionDecision: deny` on the third, naming the ticket and the first 60 chars of the command; any non-Bash tool resets the count, so it fires only on a genuine poll loop. Gated on `[ -n "$CLODEX_TICKET" ]` before reading stdin, like `bash-guard.sh`, and exits silently on a payload carrying `agent_id` — a subagent's own calls are exempt |
