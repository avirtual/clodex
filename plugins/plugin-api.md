...
and commit whatever it regenerates along with your plugin. The two artifacts'
triggers differ, and knowing which is which saves re-deriving it:
...
exports thirteen more, withheld — see the membership rule). All are `async` except
...
| `removeWorktree(worktreePath, opts)` | `{ ok, branchDeleted?, branchError? }` or `{ ok: false, error }`. Refuses to remove the main working tree — the path must be a registered *linked* worktree. With `opts.deleteBranch` it then runs `git branch -d <name>` on the repo; a refused delete (unmerged) leaves `ok: true` with `branchDeleted: null` and `branchError`. |
