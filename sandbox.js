...
function createSandbox(deps = {}) {
  ...
  async function writeComposeFile() {
    ...
    // Ensure the unconditionally-bound host source dirs exist — docker errors on
    // a bind whose source is missing. Idempotent. The `library/*` catalogues are
    // NOT created here: generateCompose skips the ones the host lacks, and
    // pre-creating them would make that check always true.
    for (const d of LIBRARY_MOUNT_DIRS) {
