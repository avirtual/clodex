...
const VERB_REGISTRY = [
  // ── daily ──────────────────────────────────────────────────────────────
  {
    ...
    notes: [
      ...
      '-n filters on each row\'s own workspace field, so it needs nothing from the node. It is ignored on the node-scoped resources (peers, teams, tickets, sandboxes, agents).',
  ...
  // ── deploy ─────────────────────────────────────────────────────────────
  {
    ...
    usage: 'deploy <user@host|ssm <name>|docker <name>|helm <name>|fargate <stack>> [flags]',
    subcommands: [
      ...
      ['deploy fargate <stack> [flags]', 'an AWS FARGATE node — cloudformation deploy of the packaged template'],
    ],
    flags: [
    ...
    examples: [
      ...
      'clodexctl deploy fargate clodex-node --subnets subnet-a,subnet-b --security-group sg-x --token-file ./token',
    ],
    notes: [
