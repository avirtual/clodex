...
const VERB_REGISTRY = [
  // ── daily ──────────────────────────────────────────────────────────────
  {
    ...
    usage: [
      ...
      'get docs [-q <terms>] [--limit N] [-o json|yaml|name]',
      'get doc <name> [--section <slug>] [-o json|yaml]',
    ...
    args: [['resource', 'sessions|workspaces|peers|teams|tickets|sandboxes|agents|docs|worktrees|catalogs|nodes (singular accepted too)'], ['name', 'one object (also accepted as session/<name>)']],
    flags: [
      ...
      ['-q, --query TERMS', 'docs only — search the help corpus instead of listing it (NAME HEADING SNIPPET)'],
      ['--limit N', 'docs only — with -q, how many hits (1-50, default 20)'],
      ['--section SLUG', 'docs only — with get doc <name>, print ONE section instead of the whole page'],
    ...
    examples: [
      ...
      'clodexctl get docs',
      'clodexctl get docs -q "park resend" --limit 5',
      'clodexctl get doc messaging --section anchors',
    ],
    notes: [
      ...
      '-n filters on each row\'s own workspace field, so it needs nothing from the node. It is ignored on the node-scoped resources (peers, teams, tickets, sandboxes, agents, docs, worktrees).',
      'Default columns: sessions NAME TYPE ACTIVITY CWD; workspaces ID NAME; peers ID LABEL ONLINE HOST VERSION; teams NAME; tickets ID TEAM STATE TITLE; sandboxes ID LABEL; agents NAME MODEL DESCRIPTION; docs NAME SECTION TITLE; worktrees PATH BRANCH HEAD.',
      ...
      'get docs lists the help corpus (the same pages the desktop Help window shows); -q searches it and prints NAME HEADING SNIPPET instead. describe doc <name> prints one page, and --section <slug> prints one section of it — a page is 10-25k tokens, a section usually under 2k.',
...

  {
    ...
    notes: [
      ...
      'The source-installed flavors (ssh/ssm) track a BRANCH and deploy no pinned artifact, so they have no target version and never no-op. Flags a context does not store REVERT on a re-run (--no-wirescope, --repo, --branch, --src, --ssh-opt, --claude-token-file) — they are named before anything runs, so pass them again if you set them. the `--ssm` deploy also MINTS A FRESH WIRE TOKEN each run: an ssm upgrade ROTATES the token, and any other holder of the old one stops being able to reach the node.',
