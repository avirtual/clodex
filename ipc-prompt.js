...
A warm-up no-op tool call first (e.g. Bash(true)) does nothing but bill a full request. Never call Bash with a bare \`true\` (or any other do-nothing command) — if you have nothing to execute, don't call a tool. COMMON MISTAKE: if your harness has a SendMessage/teammate tool, that tool reaches ONLY subagents you spawned yourself with your Agent tool — clodex agents and peers are NOT on its roster, and calling it with a clodex name just errors. The dm intent line is the ONLY transport to other clodex agents.
  ...
  [agent:team role-add <role> [prompt:<stem>] [template:<stem>] [dispatch:standing|spawn|worktree] [cwd:<rel>] [model:<id|opus|sonnet|haiku|fable>]] <brief>   Define a role on your OWN team (team lead only; every team verb is). dispatch:worktree gives every ticket to that role its own branch, tree and seat; standing (the default) routes to one long-lived seat, spawn to a one-shot one. cwd is RELATIVE to the team root. model: derives templates/<role>.json from the role's template (or clodex-team-hand) with that --model and points the role at it; a bracketed id such as claude-opus-5[1m] cannot be written here, use the alias. [agent:team role-set <role> …] takes the same kvs and patches only what you name.
...
const GRAMMAR_LINES = [
  ...
  { type: 'team-create', text: `  [agent:team create <name> root:<abs-path> [lead:<seat>]]   Mint a team manifest (privileged, operator-granted): root must be an existing absolute dir no team owns; default lead <name>-lead. Then spawn the lead there, [agent:team gather], [agent:team role-add]. [agent:team set-lead <seat>] hands the lead off (current lead only). With a body (the kickstart brief, closed by [agent:end]) the hand is made per-ticket and the brief is saved as prompts/append/team-project.md, which the stock lead and hand templates compose at boot, and Clodex spawns the lead itself in the root — do not spawn it yourself (the name is taken).` },
