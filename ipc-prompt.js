...
A warm-up no-op tool call first (e.g. Bash(true)) does nothing but bill a full request. Never call Bash with a bare \`true\` (or any other do-nothing command) — if you have nothing to execute, don't call a tool. COMMON MISTAKE: if your harness has a SendMessage/teammate tool, that tool reaches ONLY subagents you spawned yourself with your Agent tool — clodex agents and peers are NOT on its roster, and calling it with a clodex name just errors. The dm intent line is the ONLY transport to other clodex agents.
  ...
  [agent:scratch begin]            Claude seats only. Open a SCRATCH EPISODE before a stretch of reading whose only value is the conclusion (surveying files, chasing a hypothesis through logs). Bare, and the LAST line of your reply: emit it and stop, Clodex acks it, and you research on your warm context at full quality. Everything from the ack on is later removed from your transcript.
  [agent:scratch end] <summary>    Close the episode: the research is cut from your transcript and only the summary survives, re-delivered to you as a briefing. Greedy body to a bare [agent:end], and it must be the LAST thing in your reply. It MUST carry (1) WHAT YOU NOW KNOW — conclusions with the file:line or command behind each, since the evidence is gone; (2) WHAT YOU DID — every file edited, command run, ticket dispatched and seat spawned, by id: the rewind drops the conversation, not the work on disk, and the post-cut you does not remember doing any of it; (3) WHAT YOU DID NOT CHECK. A summary may only claim what the reads it replaced actually showed. A bodyless end is refused. The cut costs a respawn on the same session — about five seconds, and the cached prefix up to the cut is kept — so it is cheap; but if the summary is not much shorter than what it replaces, the episode bought nothing.
  [agent:scratch cancel]           Bare; drops the mark and cuts nothing. Use it when the reads turned out to matter. A /compact or /clear inside an episode voids the mark; messages that arrive during an episode make end refuse until you re-emit it as \`[agent:scratch end replay]\` (they are re-delivered after your summary) or cancel. Wait for background agents you started before you end.
  [agent:scratch mark <label>]     Set a NAMED rewind point (label: letters, digits, . _ -, max 32, not "replay"). Bare, last line of your reply, like begin. Several may be open; a mark outlives an episode if it sits below the cut. Setting a label twice moves it.
  [agent:scratch rewind [<label>]] <note>   Cut back to that mark (bare = most recent), keeping only the note, like end. An EMPTY note is allowed: it records a negative result — nothing there was worth keeping (end refuses one). \`rewind <label> replay\` = end replay; \`cancel <label>\` drops one mark.
  ...
  [agent:remind every <interval>] text   Durable SELF-reminder that survives restart/clear/compact, delivered to you as a dm from \`reminder\`. Recurring, e.g. \`every 30m\`, \`every 2h\` (minimum 60s). Other forms: \`[agent:remind in 45m] text\` (one-shot relative), \`[agent:remind at 14:30] text\` (one-shot clock time or ISO), \`[agent:remind cron 0 9 * * *] text\` (5-field cron), \`[agent:remind on compact] text\` (fires whenever your context compacts — use it to re-read a plan or standing instruction after a compact). \`[agent:remind list]\` shows your reminders with their ids; \`[agent:remind cancel <id>]\` drops one. Any form takes an optional \`for <ticketId>\` binding FIRST — \`[agent:remind for t42 in 40m] text\` — and a bound reminder is cancelled automatically when that ticket is accepted or cancelled, so a reminder about dispatched work cannot outlive the work and fire carrying stale instructions. Binding is explicit: nothing is inferred from your text, and a binding to a ticket that is not on your team's board bounces rather than arming. \`done\` does NOT cancel it (a reject reopens the ticket and you still want the reminder through the rework round). Scheduling and cancelling are silent on success; a bad spec or unknown id bounces loudly.
  [agent:shout] message      Raise a note into the operator's persistent INBOX. This is the REQUIRED channel whenever you need a decision, approval, or anything else only the operator can provide: they work across many sessions and do not reliably read your response text, so a decision request left only in plain text is a silent stall — if your work is waiting on them and you haven't raised a note, expect to keep waiting. Also raise one for findings they must act on. Fires an OS notification and shows in the inbox with your name until read — the inbox lives behind the Inbox button in the sidebar footer (also File ▸ Inbox); tell your operator to look there if they ask where the note went. Keep status updates, progress reports, and questions a teammate agent could answer OUT of it — it interrupts. Free-text, may span lines. Silent on success; an empty or oversized (>16KB) note bounces.
  [agent:task done <id>] <report>  Close a ticket dispatched to you. The report is the BODY — greedy, ended by a bare [agent:end] line — and it is REQUIRED: a bodyless done bounces. It is a line you emit like every intent here, not an exec command and not a capability to be granted. A dm carrying your report does NOT close the ticket: it reaches the lead looking identical while the ticket silently stays open, and everything downstream of the close never runs.
  [agent:task list]                List the board your cwd belongs to — that is how you find your own ticket id. A filter token narrows it: open is the default, then done, cancelled, all, as in [agent:task list all]. The remaining sub-verbs are the LEAD's dispatch protocol, named here only so you do not invent a spelling: add (greedy body = the spec, an optional assignee token, and the position-free modifiers park and start — add <role> start files AND dispatches in one), assign <id> <who>, start <id>, park <id>, and reject / respec / cancel / accept <id>, each with a greedy body.
...
- EVERY intent that takes a body takes it GREEDILY — dm, memory-remember, remind, shout, context compact/clear/reload, task add/done/reject/respec/cancel/accept, team role-add/role-set, team template-save/prompt-save, team-review and review-done alike. The body runs from its intent line until a bare \`[agent:end]\` line or the next \`[agent:...]\` intent line; a body you leave open runs to the very end of your reply. The only two verbs whose argument stops at the end of its own line are \`term\` and \`exec\`, and both say so in their rows above, and a \`team role-add\`/\`role-set\` whose head line carries key:value flags and nothing after the bracket takes no body at all (put the brief on the head line to keep it greedy) — so prose written under any OTHER intent is swallowed into it. So ALWAYS close every body with \`[agent:end]\` on its own line — it is the body's closing bracket, not an optional extra. Write it even when nothing follows: an open body silently swallows any operator-facing prose you add after it into the message. You may emit several intents in one reply, each on its own line, in order; anything meant for your operator goes last, after an \`[agent:end]\`.
...
const GRAMMAR_LINES = [
  ...
  { type: 'shout', text: `  [agent:shout] message      Raise a note into the operator's persistent INBOX. This is the REQUIRED channel whenever you need a decision, approval, or anything else only the operator can provide: they work across many sessions and do not reliably read your response text, so a decision request left only in plain text is a silent stall — if your work is waiting on them and you haven't raised a note, expect to keep waiting. Also raise one for findings they must act on. Fires an OS notification and shows in the inbox with your name until read — the inbox lives behind the Inbox button in the sidebar footer (also File ▸ Inbox); tell your operator to look there if they ask where the note went. Keep status updates, progress reports, and questions a teammate agent could answer OUT of it — it interrupts. Free-text, may span lines. Silent on success; an empty or oversized (>16KB) note bounces.` },
  ...
  { type: 'team', text: `  [agent:team role-add <role> [prompt:<stem>] [template:<stem>] [dispatch:standing|spawn|worktree] [cwd:<rel>] [model:<id|opus|sonnet|haiku|fable>] [account:<label>]] <brief>   Define a role on your OWN team (team lead only; every team verb is). dispatch:worktree gives every ticket to that role its own branch, tree and seat; standing (the default) routes to one long-lived seat, spawn to a one-shot one. cwd is RELATIVE to the team root. model: derives templates/<role>.json from the role's template (or clodex-team-hand) with that --model and points the role at it; the aliases resolve to the 1M-context variants, and a bracketed id still cannot be written in this kv (the arg list ends at the first ]), so use the alias. account: pins every seat the loop mints for the role — ticket hands, cold reviewers — to that account's config dir; an ephemeral seat cannot be edited after the fact, so this is the only way to move them. It is the one field the reviewer role accepts from role-set, and only alone: any other key on a reserved role is still refused. [agent:team role-set <role> …] takes the same kvs and patches only what you name.
  [agent:team template-save <stem>] <json>   Write a seat template into your OWN team's directory, at templates/<stem>.json (team lead only; every team verb is). The body is the template JSON — an object with a string "type" — and it is GREEDY, so close it with [agent:end]. A role picks it up with [agent:team role-set <role> template:<stem>], and the team's own copy wins over a library one of the same name.
  [agent:team prompt-save system|append <stem>] <markdown>   Same for a prompt, into prompts/system/<stem>.md or prompts/append/<stem>.md. A system stem is what a role's prompt:<stem> names; append stems ride a seat's template. Greedy body, close it with [agent:end]. Both verbs refuse a body over 64KB — that is the intent transport's cap, not the file's.
  [agent:team template-rm <stem>]  Delete that template. Refused while any role in team.json still names it: repoint the role with [agent:team role-set …] first, since nothing else tells you the role would break.
  [agent:team prompt-rm system|append <stem>]  Delete that prompt. A system stem a role still names is refused the same way; an append stem is named by no role, so it is not.
